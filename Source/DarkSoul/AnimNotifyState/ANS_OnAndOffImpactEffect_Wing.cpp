#include "ANS_OnAndOffImpactEffect_Wing.h"
...
void UANS_OnAndOffImpactEffect_Wing::NotifyBegin(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float TotalDuration)
{
	...
	if (OwnerActor->GetClass()->ImplementsInterface(UOwnerWeaponInterface::StaticClass()))
	{
		...
		WeaponPowerComp->SetIsImpactEffectSocketNamesActive(2, true);
	}

}

void UANS_OnAndOffImpactEffect_Wing::NotifyEnd(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation)
{
	...
	if (OwnerActor->GetClass()->ImplementsInterface(UOwnerWeaponInterface::StaticClass()))
	{
		...
		WeaponPowerComp->SetIsImpactEffectSocketNamesActive(2, false);
	}
